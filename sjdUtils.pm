...
# 2011-06-09 sjd: Add unbackslash().
# 2011-06-21 sjd: Add various xml, colorize and indent methods.
# 2011-09-13 sjd: Add unescapeURI().
# 2011-09-22 sjd: Improve indentXml().
# 2011-10-12 sjd: Fix 'default' color. Add ().
# 2011-10-19 sjd: Add escapeXmlAttribute().
# 2011-11-07 sjd: Add eWarn(), setErrorColor(), setLocs(). Flip comparison
#     in warn() functions, and make eWarn() with negative level fatal.
# 2011-12-11ff sjd: Start toHNumber(), fromHNumber(). Revamp color handling.
# 2011-12-29 sjd: Clean up, group methods topically, extend big-number stuff.
# 2012-01-05 sjd: Add escapeURI(), and fix unescapeURI().
# 2012-01-06 sjd: Add isNumeric().
# 2012-02-24 sjd: Add makePrintable(), %escape2char, %char2escape,
#     $ucLatin, $lcLatin, getUTF8() (disabled).
# 2012-03-05 sjd: Fix color and verbose handling. Add interpretLevel().
#     Keep error count.
# 2012-03-16 sjd: Hash of elements for indentXml(). Start Xterm256.
# 2012-03-28 sjd: Add isXmlName(), makeXmlName().
# 2012-04-26 sjd: Add normalizeXmlSpace(), isInteger().
# 2012-05-01 sjd: Bugfixes in makePrintable(), unbackslash().
# 2012-05-18 sjd: Add isXmlChar(), escapeXmlName().
# 2012-05-22 sjd: Add expandXML().
# 2012-05-31 sjd: Fix a couple bugs in unbackslash.
# 2012-06-04 sjd: Fix case on XML-related function names.
# 2012-06-05 sjd: Fix some color stuff. Add 'stdout' option. Add %SUoptions.
# 2012-06-11 sjd: Add precise isXmlSpaceChar isXmlNameStartChar isXmlNameChar.
# 2012-07-26f sjd: Add try_module. Add isUnicodeCodePoint().
# 2012-08-14 sjd: Use Encode, work on getUTF8(). Drop $targetFile,
#     make warnings suppress stack trace.
# 2012-09-27 sjd: Support 1000 multipliers with HNumbers, not just 1024.
# 2012-10-16ff sjd: Compile XML char-set regexes. Add aliases vMsg, eMsg, hMsg.
#     Add unescapeXmlName(). Add hash arg to expandXml().
# 2012-10-23 sjd: Improve showInvisibles, give space dispay options.
# 2012-11-01 sjd: Add indentJson(). Clean up w/e/h message i/f a bit.
# 2012-11-19 sjd: Add escapeXmlPi(), escapeXmlComment().
# 2012-11-26 sjd: Add localize() and update lpadc() for it.
# 2012-12-03 sjd: Drop setXColor() methods. Add MsgType(). Move color and
...
# 2012-12-18 sjd: Add 'asciiOnly' param to escapeXML... methods. escapeJson().
# 2013-01-04 sjd: Fix asciiOnly. Add 'sep' param to getUTF8().
# 2013-02-06 sjd: Add vPush()/vPop() (mainly for grepData command).
# 2013-02-25 sjd: Improve indentXml().
# 2013-06-05 sjd: Add MsgType 'prefix' and 'suffix'.
# 2013-06-18ff sjd: Add whereAmI(). Lose [ehv]Warn() (keep [ehv]Msg()).
# 2013-06-27: Add lorem(), vis().
# 2013-08-27: Add isUpper(), isLower(), isCapitalized(), sc(), tc(), ssubstr().
# 2013-08-29: Clean up msgType handling.
#     Rename vPush/vPop to MsgPush/MsgPop, MsgType->Msg, setMessagesType... to
#     defineMsgType etc. Message option to do showInvisibles().
# 2013-12-18: Add defineMsgType param for whether to indent per vpush/vpop.
# 2014-06-11: XML functions get aliases for both "Xml" and "XML".
#     Add availableFileNum().
# 2014-07-17: Resync w/ Python ver. Nuke SUset etc. Separate options vs. info.
# 2014-09-01ff: Add LF choices to showInvisible. Document makePrintable().
#     Add ulorem option and ulorem().
# 2015-03-26ff: Some sync to Python version enhancements: Discard locs,
...
# 2015-08-28: Add uncolorize(), make pline() ignore color escapes.
# 2015-09-11: Fix makePrintable to handle SP and LF consistently.
# 2016-04-07f: Add trim(), ltrim(), rtrim(), cpad(). Avoid refs %colorStrings.
#     'underline'->'ul', 'reverse'->'inverse', to match colorstrings command.
# 2016-07-21: Create ColorManager package. Sync color w/ python, logging....
# 2016-10-25: Physically move ColorManager package out.
#     Drop ulorem option, ulorem(), and xlorem().
# 2018-03-27: Split alogging to separate package, like Python version.
# 2018-09-35: Add splitPath(), availableFileName().
...
#
# Low priority:
#     Pull in grepData::Condition::getDate().
#     Pull in struct pretty-printer from volsunga TrellisNode.
#     Pull in basic Unicode char normalization from findExamples.py.
#     Support escaping to HTML entities, leaving Latin-1,....
#     Drop rarely-used stuff (in sync w/ Python version, too).
#
package sjdUtils;
...
Pad I<string> on both ends (thus, approximatig centering it)

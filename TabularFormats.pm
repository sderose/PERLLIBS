...
# 2012-05-29ff sjd: Pull parsestringtoDOM() from FakeParser.pm. Shift methods
...
# 2014-02-05ff: Fix setFieldNamesFromArray to fully create fields as needed.
#     Drop DataCurrent package entirely.
# 2015-02-19: Drop 'recover' params for dealing with missing field name/number.
#     Instead just return undef and let caller deal.
# 2017-04-19: Split DataSchema and FieldDef to separate file TableSchema.pm,
#     and DataSource to separate file DataSource.pm,
# To do:
#     Why continuous (-v) warnings about re-adding same fields?
#     Make each file format be its own subclass, allocated directly.
#     Split subclasses into separate files.
#     Add supportsFieldNames().
#     Add formats for graphviz drawing links keyed by 2 fields? Separate app?
...
#     Rename options to start with format they apply to?
#     Fix handling of XSV headers.
#     Finish padding/alignment for output, in FieldDef::align.
...
#     Integrate into C<makeHTMLtable>, C<makeNMIgraph>.
#     FormatSniffer.
#
# Format-specific:
...
#     Should chooseFormat go in TFormatSupport? Maybe ditch subclassing there?
#
